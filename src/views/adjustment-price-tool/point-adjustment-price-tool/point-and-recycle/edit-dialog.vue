...
<template>
  <div>
    <el-dialog :close-on-click-modal="false" :visible.sync="editObj.isShow" title="新增点数与回收率" @close="cancelDialog">
          ...
          <el-col :span="12">
            <el-form-item label="统筹区" prop="poolarea" >
              <el-select v-model="ruleForm.poolarea" clearable placeholder="请选择统筹区" style="width:100%;" >
                <el-option v-for="item in editObj.poolareaList" :key="item.value" :label="item.label" :value="item.value" />
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="12">
    ...
    submitDialog() {
      this.$refs['ruleForm'].validate((valid) => {
        if (valid) {
          ...
          // console.log(1)
          API.savePricServitemPt(params).then(res => {
            if (res.data.code === 0) {
              this.$message({
                type: 'success',
